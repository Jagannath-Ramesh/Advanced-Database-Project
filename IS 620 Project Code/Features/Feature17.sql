create or replace
procedure search_multiple (usr_id in messages.user_id%type, key_in KEYWORDS) is
begin
for i in 1 .. key_in.count loop
  search_using_word(usr_id, key_in(i));
end loop;
end;

create or replace
procedure search_using_word ( usr_id users.user_id%type, In_Word varchar2)
Is
